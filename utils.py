...
class MultiAgentStateRecorder(gym.Wrapper):
    """
    A gym wrapper that records episode statistics for multi-agent environments
    """
    def __init__(self, env: gym.Env):
        super().__init__(env)
        # Initialize trackers that will be reset at the start of each episode
        self._per_agent_rewards: Dict[str, float] = defaultdict(float)
    ...
    def reset(self, **kwargs) -> Any:
        ...
        self._per_agent_rewards.clear()
        self._episode_length = 0
        self._episode_start_time = time.monotonic()

        return obs

    def step(self, action: Any) -> Tuple[Any, Dict[str, float], Dict[str, bool], Dict[str, Any]]:
        """
        Steps the environment, updates statistics, and adds them to `info` if the episode ends.
        """
        ...
        for agent_id in range(len(rewards)):
            self._per_agent_rewards[agent_id] += rewards[agent_id]

        if done:
            # Calculate final statistics
            total_reward = sum(self._per_agent_rewards.values())
            elapsed_time = time.monotonic() - self._episode_start_time

            # Add statistics to the info dictionary
            info["episode"] = {
                "per_agent_rewards": self._per_agent_rewards.copy(),
...
def make_env(config):
    if 'MultiGrid' in config.domain:
        ...
        env = MultiAgentStateRecorder(env)
    else:
...
def plot_single_frame(frame_id, full_env_image, agents_partial_images, actions, rewards, action_dict,
                      fig_dir, expt_name, figsize=(10,10), predicted_actions=None, shared_ylim=False, min_ylim=.0001):
    # Seaborn palette.
    sns.set()
